...
#define PROFILES_DIR "/etc/securenux_profiles/"
#define PROFILES_CONFIG "/etc/securenux_priority"
...
int conacat(char a, char b, char *c) {
    if (sizeof(*c) < sizeof(*a) + sizeof(*b) + 1) return -1;
    strcpy(*c, a);
    strcat(*c, b);
    return 0;
}
...
static int merge_profiles(struct profile_data *merged, char **profile_files, 
    ...
    for (int i = 0; i < num_files && merged->rule_count < MAX_RULES_PER_PROFILE; i++) {
        ...
        if (load_profile(profile_files[i], &temp_profile) == 0) {
            for (int j = 0; j < temp_profile.rule_count && merged->rule_count < MAX_RULES_PER_PROFILE; j++) {
                temp_profile.rules[j].priority = num_user_profiles + num_files - 1 - i;
    ...
    for (int i = 0; i < num_user_profiles && merged->rule_count < MAX_RULES_PER_PROFILE; i++) {
        ...
        if (load_profile(user_profiles[i], &temp_profile) == 0) {
            for (int j = 0; j < temp_profile.rule_count && merged->rule_count < MAX_RULES_PER_PROFILE; j++) {
                temp_profile.rules[j].priority += num_user_profiles - 1 - i;
                merged->rules[merged->rule_count++] = temp_profile.rules[j];
            }
        }
    }
...
static int load_system_profiles(char ***profile_files, int *num_files, char *) {
    ...
    *num_files = 0;
    ...
    while (fgets(line, sizeof(line), config) && *num_files < count) {
        ...
        if (filename[0] != '/') if (conacat(*filename, PROFILES_DIR, filename) != 0) {
            fprintf(stderr, "Error in '%s' on line %d: line is too long. (Maximum is %d.)", PROFILES_CONFIG, *num_files, MAX_LINE_LEN);
            return -1;
        }
